...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", dest="output_prefix", type=str, default="synthetic_output"
    )
    ...
    for eps in lst_eps:
        ...
        print(f"Viterbi Accuracy (knows parameters): {viterbi_accuracy}", file=log_fd)
        print(f"SB Attacker: {np.sum(latent==sanitized)/len(latent)}")

        for inner_idx in range(seq_len - size - 2):
            inputs[inner_idx] = torch.tensor(sanitized[inner_idx : inner_idx + size])
