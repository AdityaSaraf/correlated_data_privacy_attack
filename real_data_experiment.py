...
if __name__ == "__main__":
    ...
    parser.add_argument("--data", dest="data", type=str, default="data/normal/N8")
    parser.add_argument(
        "--output", dest="output_prefix", type=str, default="real_data_output"
    )
    ...
    q = transition_counts[(False, True)] / (
        transition_counts[(False, True)] + transition_counts[(False, False)]
    )
    r = transition_counts[(True, False)] / (
        transition_counts[(True, False)] + transition_counts[(True, True)]
    )
    ...
    for eps in lst_eps:
        ...
        print(f"SB acc:{np.sum(latent == sanitized)/latent.shape[0]}", file=sys.stderr)
        print(f"SB acc:{np.sum(latent == sanitized)/latent.shape[0]}", file=log_fd)
        ...
        for train_idx, test_idx in lstm_attack.k_folds(num_folds, len(all_data)):
            ...
            fold_idx += 1
            training_data = torch.utils.data.Subset(all_data, indices=train_idx)
